...
**Deterministic fallback:** `buildDeterministicCoachingResponse()` generates a coaching reply from the pre-computed intelligence context signals without any AI call. This exists for two reasons: it ensures the coaching feature degrades gracefully when `ANTHROPIC_API_KEY` is absent or invalid, and it provides a testable baseline for the coaching output format. The fallback is word-by-word streamed to the client using the same `ReadableStream` interface as the Claude path, so the UI behaves identically.
...
Moments I overrode AI output: the INTERVAL workout classifier initially used a 1-lap minimum, which misclassified multi-effort tempo sessions as intervals; the fix required understanding what an interval session structurally is (repeated short hard efforts with rest, not a single hard lap) and correcting the classifier's lap-count threshold. The Prisma v6 pin was a judgment call I made explicitly against what Cursor and Claude Code would have defaulted to — they both defaulted toward the latest version and needed explicit instruction to stay on v6.
